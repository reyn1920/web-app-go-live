...
def run_command(
    cmd: List[str], cwd: Optional[Union[str, os.PathLike[str]]] = None, env: Optional[Dict[str, str]] = None
) -> Tuple[bool, str, str]:
    """Run a shell command and return (success, stdout, stderr)."""
    try:
        if isinstance(cmd, str):
            cmd = shlex.split(cmd)
        subprocess.run(cmd, capture_output=True, text=True, cwd=cwd, env=env, check=False)
...
def main() -> None:
    ...
    # runtime import from backend/ directory
    ...
    # runtime package import from root
